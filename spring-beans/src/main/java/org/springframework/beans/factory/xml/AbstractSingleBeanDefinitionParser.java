...
public abstract class AbstractSingleBeanDefinitionParser extends AbstractBeanDefinitionParser {
	...
	protected final AbstractBeanDefinition parseInternal(Element element, ParserContext parserContext) {
		...
		if (beanClass != null) {
			...
			// 对于 getBeanClass() 方法，AbstractSingleBeanDefinitionParser 类并没有提供具体实现，而是直接返回 null ，意味着它希望子类能够重写该方法。
			// 当然，如果没有重写该方法，这会去调用 #getBeanClassName() ，判断子类是否已经重写了该方法
		...
		// 调用子类的 doParse() 进行解析 【空实现】
		doParse(element, parserContext, builder);
		return builder.getBeanDefinition();

		// 所以对于该方法，期待子类实现 getBeanClass() 和 doParse() 方法尤为重要！提供该方法的实现，才能解析自定义标签
	}
