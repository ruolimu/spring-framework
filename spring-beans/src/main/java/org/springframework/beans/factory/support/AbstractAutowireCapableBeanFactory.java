...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		try {
			...
			// <lookup-method> 和 <replace-method> 解析过程其实就是这两个配置存放在 BeanDefinition 中的 methodOverrides 属性中
		...
		try {
			...
			// <3> 实例化的前置处理
			// 给 BeanPostProcessors 后置处理器返回一个代理对象 bean 的机会
			// 【AOP 的功能就是基于这个地方判断的】
	...
	protected Object doCreateBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		synchronized (mbd.postProcessingLock) {
			if (!mbd.postProcessed) {
				try {
					// 后置处理修改 BeanDefinition (MergedBeanDefinitionPostProcessor 的应用)
	...
	protected BeanWrapper createBeanInstance(String beanName, RootBeanDefinition mbd, @Nullable Object[] args) {
		...
		// <1> 如果存在 Supplier 回调
		// Supplier 接口的作用：指定创建 bean 的回调。如果我们设置了这样的回调，那么其他的构造器或者工厂方法都会没有用
		// Supplier属性 在 RootBeanDefinition extends AbstractBeanDefinition 的构造方法时设置
		Supplier<?> instanceSupplier = mbd.getInstanceSupplier();
		if (instanceSupplier != null) {
			// 如果设置了 instanceSupplier 属性，使用给定的回调方法初始化策略
			return obtainFromSupplier(instanceSupplier, beanName);
		}

		// <2> 使用 FactoryBean 的 factory-method 来创建，支持静态工厂和实例工厂
		if (mbd.getFactoryMethodName() != null) {
			// 构造一个 ConstructorResolver 对象
	...
	protected void populateBean(String beanName, RootBeanDefinition mbd, @Nullable BeanWrapper bw) {
		...
		// <2> 是否自动注入
		int resolvedAutowireMode = mbd.getResolvedAutowireMode(); // 根据该值判断
	...
	protected void autowireByName(
		...
		// <1> 对 Bean 对象中的 非简单属性
		// 非简单属性：类型为对象类型的属性，但是这里并不是将所有的对象类型都都会找到
	...
	protected void applyPropertyValues(String beanName, BeanDefinition mbd, BeanWrapper bw, PropertyValues pvs) {
		...
		if (pvs instanceof MutablePropertyValues) {
			...
			if (mpvs.isConverted()) {
				...
				try {
					// todo 【★★★】为实例化对象设置属性值 ，依赖注入真真正正地实现在此！！！！！
	...
	protected Object initializeBean(String beanName, Object bean, @Nullable RootBeanDefinition mbd) {
		...
		// <3> 激活用户自定义的 init 方法
		// <bean> 标签的配置中 init-method 方法就是在这里执行的，此外也可实现 InitializingBean 接口
		try {
