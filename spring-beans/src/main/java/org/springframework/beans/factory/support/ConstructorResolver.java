...
class ConstructorResolver {
	...
	/**
	 * 总结：首先确定构造函数参数、构造函数，然后调用相应的初始化策略进行 bean 的初始化
	 *
	...
	/**
	 * 总结：确定工厂对象，然后获取构造函数和构造参数，最后调用 InstantiationStrategy 对象的 #instantiate() 方法，来创建 Bean 实例
	 *
	...
	public BeanWrapper instantiateUsingFactoryMethod(
		...
		// <2.1> 如果指定了构造参数 explicitArgs 则直接使用
		// explicitArgs 参数，是我们调用 #getBean(...) 方法时传递进来的。
		// 一般该参数，该参数就是用于初始化 Bean 时所传递的参数。
		// 如果该参数不为空，则可以确定构造函数的参数就是它了。
		...
		if (factoryMethodToUse == null || argsToUse == null) {
			...
			// 排序构造函数
			// 排序规则：public 构造函数优先参数数量降序，非 public 构造函数参数数量降序
			...
			if (explicitArgs != null) {
				...
				// <2.3.1> getBean() 没有传递参数，则需要解析保存在 BeanDefinition 构造函数中指定的参数
				if (mbd.hasConstructorArgumentValues()) {
					// <2.3.1> 构造函数的参数
					ConstructorArgumentValues cargs = mbd.getConstructorArgumentValues();
					resolvedValues = new ConstructorArgumentValues();
					// <2.3.2> 解析构造函数的参数
					// 将该 bean 的构造函数参数解析为 resolvedValues 对象，并返回解析到的参数个数 minNrOfArgs，其中会涉及到其他 bean
					minNrOfArgs = resolveConstructorArguments(beanName, mbd, bw, cargs, resolvedValues);
				}
				else {
					minNrOfArgs = 0;
			...
			for (Method candidate : candidates) {
				...
				if (parameterCount >= minNrOfArgs) {
					// 保存参数的对象
					// 将对象包装成 ArgumentsHolder 对象后，我们就可以通过它来进行构造函数匹配
					ArgumentsHolder argsHolder;
