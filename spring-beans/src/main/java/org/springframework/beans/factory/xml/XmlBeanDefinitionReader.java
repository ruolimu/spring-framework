...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	protected EntityResolver getEntityResolver() {
		if (this.entityResolver == null) {
			...
			// 默认创建一个 DelegatingEntityResolver 对象。该 Resolver 委托给默认的 BeansDtdResolver 和 PluggableSchemaResolver
			else {
	...
	public int loadBeanDefinitions(Resource resource) throws BeanDefinitionStoreException {
		//这里将 Resource → EncodedResource，主要是为了对 Resource 进行编码，保证内容读取的正确性
	...
	/**
	 * 真正将 XML file 加载为 bean definitions 的过程
	...
	protected Document doLoadDocument(InputSource inputSource, Resource resource) throws Exception {
		//获取指定资源（xml）的验证模式，调用 #getValidationModeForResource(Resource resource) 方法
		//为什么需要获取验证模式呢？ XML 文件的验证模式保证了 XML 文件的正确性

		//获取 XML Document 实例，调用 DocumentLoader#loadDocument(InputSource inputSource, EntityResolver entityResolver, ErrorHandler errorHandler, int validationMode, boolean namespaceAware) 方法
	...
	public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
		...
		// <4> 创建 XmlReaderContext 对象，读取 XML 元素，注册 BeanDefinition 们
