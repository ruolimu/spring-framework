...
public class BeanDefinitionParserDelegate {
	 ...
	 * <4> 处，对属性进行解析并封装成 AbstractBeanDefinition 实例 beanDefinition
	...
	/**
	 * 注意：meta 所声明的 key 并不会在 Bean 中体现，只是一个额外的声明，当我们需要使用里面的信息时，通过调用 BeanDefinition 的 #getAttribute(String name) 方法来获取
	...
	public void parseMetaElements(Element ele, BeanMetadataAttributeAccessor attributeAccessor) {
		...
		for (int i = 0; i < nl.getLength(); i++) {
			...
			if (isCandidateElement(node) && nodeNameEquals(node, META_ELEMENT)) { // 标签名为 meta
				...
				// 委托 AttributeAccessorSupport 实现，是接口 AttributeAccessor 的实现者。 AttributeAccessor 接口定义了与其他对象的元数据进行连接和访问的约定，可以通过该接口对属性进行获取、设置、删除操作
				// 注意：BeanMetadataAttributeAccessor extends BeanMetadataAttributeAccessor  extends AttributeAccessorSupport
	...
	public void parsePropertyElements(Element beanEle, BeanDefinition bd) {
		NodeList nl = beanEle.getChildNodes();
		// “提取”( 遍历 )所有的 property 的子元素，然后调用 parsePropertyElement() 提取
		for (int i = 0; i < nl.getLength(); i++) {
			Node node = nl.item(i);
			...
			try {
				...
				if (index < 0) {
					...
					try {
						...
						// <4> 将 ValueHolder 实例对象添加到 加入到 indexedArgumentValues 集合中
						else {
	...
	public void parsePropertyElement(Element ele, BeanDefinition bd) {
		...
		try {
			...
			// 根据该值构造 PropertyValue 实例对象
			PropertyValue pv = new PropertyValue(propertyName, val);
			parseMetaElements(ele, pv);
			pv.setSource(extractSource(ele));
			// 将 PropertyValue 添加到 BeanDefinition 中的 MutablePropertyValues 中
	...
	public Object parsePropertyValue(Element ele, BeanDefinition bd, @Nullable String propertyName) {
		...
		// <2> 若存在 ref 属性，将 ref 属性值，构造为 RuntimeBeanReference 实例对象
		if (hasRefAttribute) {
		...
		// <3> 若存在 value 属性，将 value 属性值，构造为 TypedStringValue 实例对象
		else if (hasValueAttribute) {
			TypedStringValue valueHolder = new TypedStringValue(ele.getAttribute(VALUE_ATTRIBUTE));
			valueHolder.setSource(extractSource(ele));
			return valueHolder;
		}
		// <4> 解析子元素，如果子元素不为空，则对子元素进一步解析
	...
	public BeanDefinition parseCustomElement(Element ele, @Nullable BeanDefinition containingBd) {
		...
		//  	调用 XmlReaderContext#getNamespaceHandlerResolver() 方法，获得命名空间的解析器 NamespaceHandlerResolver
		// 		调用 NamespaceHandlerResolver#resolve(String namespaceUri) 方法，根据 namespaceUri 获取相应的 Handler 对象
	...
	public BeanDefinitionHolder decorateBeanDefinitionIfRequired(
		...
		// <1> 遍历属性，查看是否有适用于装饰的【属性 attributes】
		...
		// <2> 遍历子节点，查看是否有适用于修饰的【子节点 childNodes】
	...
	/**
	 * 通过名称空间处理程序装饰给定的bean定义，装饰对应的节点(Node)
